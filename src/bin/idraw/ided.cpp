...
inline PulldownMenu* MakePulldown (const char* name) {
    return new PulldownMenu(
        new HBox(
            new Message(name, Center, iv26_round(.1*cm)),
...
Interactor* IdrawEditor::Tools () {
    ...
    ArrowLine* line = new ArrowLine
      (0, 0, unit, unit, (boolean)false, (boolean)false, 1., stdgraphic
    );
    ...
    ArrowMultiLine* ml = new ArrowMultiLine
      (xOpen, yOpen, nOpen, (boolean)false, (boolean)false, 1., stdgraphic
    );
...
void IdrawEditor::InitViewer () {
     ...
     */
    const float w = iv26_round(atof(page_w) * inches);
    const float h = iv26_round(atof(page_h) * inches);
...
PulldownMenu* IdrawEditor::BrushMenu () {
    ...
    while (br != nil) {
        ...
        if (br->None()) {
            ...
            line = new ArrowLine
	      (0, 0, iv26_round(MENU_WIDTH*cm), 0, (boolean)false, (boolean)false, 1., stdgraphic
	    );
            line->SetBrush(br);
    ...
    line = new ArrowLine(
	0, 0, iv26_round(MENU_WIDTH*cm), 0, true, false, 1., stdgraphic
    );
    ctrlInfo = new ControlInfo(new ArrowLineComp(line));
    Include(new ArrowCmd(ctrlInfo, true, false), pdm);

    line = new ArrowLine(
	0, 0, iv26_round(MENU_WIDTH*cm), 0, false, true, 1., stdgraphic
    );
    ctrlInfo = new ControlInfo(new ArrowLineComp(line));
    Include(new ArrowCmd(ctrlInfo, false, true), pdm);

    line = new ArrowLine(
	0, 0, iv26_round(MENU_WIDTH*cm), 0, true, true, 1., stdgraphic
    );
    ctrlInfo = new ControlInfo(new ArrowLineComp(line));
    ...
    while (pat != nil) {
        ...
        if (pat->None()) {
            ...
            SF_Rect* sfr = new SF_Rect(
                0, 0, iv26_round(MENU_WIDTH*cm), iv26_round(MENU_HEIGHT*cm), stdgraphic
            );
            sfr->SetPattern(pat);
...
PulldownMenu* IdrawEditor::ColorMenu (const char* name, const char* attrib) {
    ...
    while (color != nil) {
        ControlInfo* ctrlInfo;
        Coord w = iv26_round(MENU_WIDTH*cm);
        Coord h = iv26_round(MENU_HEIGHT*cm);
